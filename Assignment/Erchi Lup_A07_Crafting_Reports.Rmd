...
# 1. Compute mean PM2.5 concentration for each county, sorted and rounded
...
# 2. Compute mean ozone concentration for each county, sorted and rounded
...
# 3. Use kable to produce the PM2.5 table
kable(summary_PM25, 
      caption = "Mean Particulates (2.5mm)", 
      col.names = c("County", "$\\mu g/m^3$"))

# 4. Use kable to produce the Ozone table
kable(summary_Ozone, 
      caption = "Mean Ozone", 
      col.names = c("County", "$\\mu g/m^3$"))
...
```{r ozone_2018, echo=FALSE, fig.align='left', fig.height=3.5, fig.cap="2018 Monthly Ozone Distribution"}
epa_data %>% 
  filter(year(Date) == 2018) %>% 
  drop_na(Ozone) %>% 
  ggplot(aes(x = factor(month(Date), levels=1:12, labels=month.abb), y = Ozone)) +
  geom_boxplot() +
  theme_minimal()
```

```{r zone_2019, echo=FALSE, fig.align='left', fig.height=3.5, fig.cap="2019 Monthly Ozone Distribution"}
epa_data %>% 
...
## Git Repository
https://github.com/luoerchi-maker/EDE_Spring2026/commits/main/
